...
**最近一次真机整跑**（2026-09-19，四段同口径：46 页论文 PDF + RGB 官方 200 条）：

| 指标 | 结果 |
|---|---|
| 答案含期望事实 / 引用忠实度 / 引用页码 | **100%** / **100%** / **100%** |
| RAGAS faithfulness / context_precision | **0.698** / **0.935**（裁判 `qwen-max`, temp=0） |
| 检索层 hybrid+rerank hit@10 / MRR | **0.92** / **0.667**（明显高于另三路） |
| RGB 四能力 事实命中 | **81%**（噪声 86% / 信息集成 66% / 反事实 92%） |
| RGB 拒答率（负样本） | 36% |
| 延迟（4 并发）首字 P50 / P95 | 5606 ms / 18995 ms |

📄 **完整一页报告**（口径、目标线对照、已知边界都在里面）：[docs/eval-report.md](docs/eval-report.md)

> **口径**：判据为「去空白、小写后子串匹配」，兼容 Docling 在数字与标点间插入空格（如 `表 4 . 1`）。
> `evaluate.py` 默认取 `backend/paper.pdf`（自备文档，未入库）；跑自己的文档请设 `EVAL_DOC`。
> ⚠️ **`引用忠实度 100%` 是检索侧的判据**（期望事实在不在返回的来源里），**不等于「答案 100% 有据」**。
> 「论断被来源支撑」的真实水平看**引用覆盖率 59%** 与 **RAGAS faithfulness 0.698** —— 两种独立测法互相印证。

> **重排降级**：重排服务不可达时默认**降级为 RRF 原顺序**（不打断回答），但这个事实会一路带进
> `done` 事件与 trace，报告里也会写明「有 N 次查询降级」。跑评测想要更硬的口径就设 `RERANK_STRICT=true`
> —— 那样重排挂掉直接报错、那一段照惯例写「未跑」，绝不出一批「看着像重排跑过」的数字。
...
- ✅ **带数字的一页评测报告**：[docs/eval-report.md](docs/eval-report.md)（2026-09-19 真机整跑，四段同口径，含口径与已知边界）。
- 🚧 **仍未做的交付物**（详见 [docs/ROADMAP.md](docs/ROADMAP.md)）：**在线 Demo 地址**、**2 分钟演示视频**。
- ⚠️ **边界**：
